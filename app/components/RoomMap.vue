            ...
            <div class="maptiles-pan" :style="mapStyle">
                <div class="maptiles1" :style="tilesScaleStyle">
                    ...
                    <OtherCharacter
                        ...
                        :back-hair="otherCharByUserId[other.userId]?.backHair"
                        :top-ratio="topRatio"
                        :local-x="other.x"
                        :local-y="other.y"
                        :local-z="other.z ?? 0"
...
//
// 좌표계 변환(타일 좌표 → 캐릭터 로컬 좌표)은 useIsoMap.ts의 tileToLocal 참고 — 그대로
// (tx,ty)를 캐릭터 위치에 넣으면 엉뚱한 화면 좌표로 계산돼서(실제로 스폰을 (2,2)로 찍었더니
// 캐릭터가 맵과 동떨어진 허공에 뜨는 버그가 있었음) 반드시 변환을 거쳐야 함
function getSpawnPoint() {
    const spawn = mapInfo.value?.[2]
    if (!spawn || typeof spawn.x !== 'number' || typeof spawn.y !== 'number') return { x: 0, y: 0 }
    return tileToLocal(spawn.x, spawn.y)
}
...
/* 모바일 전용 이동 조이스틱: 기본 숨김, 768px 이하에서만 노출. 점프 버튼(오른쪽)과 반대쪽인
   왼쪽에 둠 — 오른손 엄지로 점프, 왼손 엄지로 이동하는 일반적인 모바일 조작 배치 */
#mobile-joystick {
    display: none;
    position: absolute;
    left: 20px;
