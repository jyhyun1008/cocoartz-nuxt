...
const character = "/character/sample/1.png"
...
const { data: roomData, error } = await useAsyncData(
    roomKey, async () => {
        ...
        const response = await $fetch(`${apiBaseUrl}/api/getRoomByPath`, {
            method: "POST",
            headers: {
...
const getScreenPosition = (tile) => {
  const style = {
    top: `calc(50dvh - 1.5rem - ${-24 + tile.position.y*36 + tile.position.x*(-36)}px)`,
