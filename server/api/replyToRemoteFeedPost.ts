...
export default eventHandler(async (event) => {
    ...
    if (verificationRequired) {
        ...
        if (!author || !isVerified(author, verificationRequired)) {
            throw apiError(403, 'REMOTE_COMMENT_EMAIL_VERIFICATION_REQUIRED', '연합 게시판은 이메일 인증을 완료해야 댓글을 쓸 수 있어요')
        }
    }

    const [feedPost] = await db.select().from(remoteTimelinePosts).where(eq(remoteTimelinePosts.id, remoteFeedPostId))
    if (!feedPost) throw apiError(404, 'POST_NOT_FOUND', '글을 찾을 수 없습니다')

    const [user] = await db.select().from(users).where(eq(users.id, userid))
    const actor = await ensureActor(userid)
    if (!user || !actor) throw apiError(500, 'REPLY_SEND_FAILED', '답글을 보낼 수 없습니다')
