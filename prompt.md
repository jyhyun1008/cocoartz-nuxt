...
| 5 | 멀티유저 | 채팅 메시지별 이모지 리액션 (게시판과 동일한 방식, HTTP 로드분/WS 실시간분 모두 대응) | ✅ |
| 4 | UI | 전역 라이트/다크 모드 테마 시스템 (`--fg-rgb`/`--surface-*`/`--accent-fg-rgb` 등 CSS 변수, 명도 기반 자동 대비) | ✅ |
| 4 | UI | 개인 설정 페이지 (`/preferences`) — 라이트/다크 테마 전환 + 원격 계정 팔로우 관리 (헤더의 테마 토글 버튼은 이걸로 대체되며 제거) | ✅ |
| 4 | UI | 전체 유니코드 이모지 피커 (게시판 리액션 + 게시글/위키/채팅 입력 공용), 원격 커스텀 이모지(`:shortcode:`) 렌더링 | ✅ |
| 4 | 유저 | 프로필 배너 이미지 업로드 (아바타와 동일 패턴) | ✅ |
| 4 | 유저 | 마을/공지 게시판 등 고정 채널 이름 변경 (헤더에도 즉시 반영) | ✅ |
| 3 | 게시판 | 게시물 수정 / 삭제 (수정은 비연합 채널만, 삭제는 전체) | ✅ |
| 3 | 게시판 | 게시판 페이지네이션 (20개씩, 더보기 버튼) | ✅ |
| 3 | 게시판 | Content Warning(CW) 열람주의 게이트 | ✅ |
| 5 | 멀티유저 | 팔로우 기반 개인 타임라인 (`/timeline`) — 로컬+원격 팔로잉 글 + 내 글, 게시판과 동일한 목록/상세보기 구조 | ✅ |
| 6 | 연합 | 페디버스 액터/웹피거/HTTP 시그니처 기반 ActivityPub 인프라 | ✅ |
| 6 | 연합 | 아웃바운드 연동 (Create/Update/Delete, Follow/Accept/Undo, Like/Announce) | ✅ |
| 6 | 연합 | 인바운드 수신 — 연합 게시판 원격 답글 + 팔로우 기반 개인 피드 저장 | ✅ |
| 6 | 연합 | 인바운드 공개 범위(`to`/`cc`의 Public 여부) 판정 — 팔로우 기반 원격 원본 글(`remoteFeedPosts`, 아직 대화 맥락 없는 게시물)만 판정해서 `isPublic`으로 저장. 개인 타임라인(`getFollowingFeed`)은 공개 범위 무관 전부 노출, 다들 보는 연합 게시판(`getRemoteFeedPosts`)은 `isPublic=true`만 필터링. 반대로 로컬↔원격 답글(스레드)은 이미 공개인 부모 글에 달린 대화라 공개 범위와 무관하게 양방향 전부 수신 (양방향 답글 스레드가 목표라 여기서 거르지 않음) | ✅ |
| 6 | 연합 | 원격 서버 배지 (실제 로고/테마색, `/manifest.json` 기반) | ✅ |
| 6 | 연합 | 이모지 리액션은 연동되지 않음 - 좋아요만 연동 | ✅ |
| 6 | 연합 | 연합 게시판에 뜬 팔로우 피드(원격) 글에 좋아요(Like/Undo)·댓글(답글로 원 작성자에게 배포, `posts.remoteParentObjectId`로 로컬 글타래 없이 추적) | ✅ |
| 7 | 상점 | 상점 채널 UI | ⬜ |
