...
| 6 | 연합 | 인바운드 공개 범위(`to`/`cc`의 Public 여부) 판정 — 연합 게시판에 직접 오는 답글은 수신 시점에 홈 공개/팔로워 공개 등을 아예 거부(전체공개+조용히 공개만 저장). 팔로우 기반 원격 글(`remoteFeedPosts`)은 판정 결과를 `isPublic`으로만 저장해두고, 개인 타임라인(`getFollowingFeed`)에서는 공개 범위 무관 전부 노출, 다들 보는 연합 게시판(`getRemoteFeedPosts`)에서는 `isPublic=true`만 필터링해서 노출 | ✅ |
